...
def make_ols_query(term_uri, ontology_id):
    """
    ...
    """
    results = requests.get(f"{BASE_URL}ontologies/{ontology_id}/terms?iri={term_uri}").json()
    print(results)
    if 'error' in results:
        print
        return None
...
def get_ontology_id(term_uri):
    """
    Extracts the ontology id from the term uri, to be used for OLS queries by reading the last part of an iri and 
